...
\item
  ...
  \textbf{efficiently} \emph{iterative} (can be found in Lecture 6) and \emph{recursive} binary tree
...
\item
  ...
  Therefore, you need to define transformation \textbf{T1} from the
    ...
    Pretty Print (\textbf{PP}) display (see page 3).
...
\item
  Write your observations in the header (block
...
\subsubsection{\texorpdfstring{Implementation of \emph{iterative} (\emph{with constant additional memory}) and
\emph{recursive} binary tree traversal in \emph{O(n)}
...
Only one tree traversal order is required— Preorder, Inorder, or Postorder—and you may choose whichever you prefer.

You may generate a sorted array that can be transformed into a binary tree through a recursive approach.

\subsubsection{\texorpdfstring{Transformations between tree representations}{Transformations between tree representations}}\label{transformations-between-tree-representationsp}


\begin{itemize}
  \item[1.] \textbf{Correct implementation for Pretty-print for R1 (2p)}  
  
  \emph{Demo}: The correctness of the algorithms should be demonstrated using the example  
  $\Pi = \{2,7,5,2,7,7,-1,5,2\}$ with a pretty-print, as shown in the Figure below.

  \item[2.] \textbf{Correct implementation for \emph{T1} (from R1 to R2) and pretty-print for \emph{R2} (1p) + \emph{T1} in linear time (1p)}  
  
  \emph{Demo}: The correctness of the algorithms should be demonstrated using the example  
  $\Pi = \{2,7,5,2,7,7,-1,5,2\}$ with a pretty-print, as shown in the Figure below.

  \item[3.] \textbf{Correct implementation for \emph{T2} (from R2 to R3) and pretty-print for \emph{R3} (2p) + \emph{T2} in linear time (1p)}  
...
Use Pretty Print for all three representations.  
Each representation (R1, R2, R3) should have a pretty print of its own with a different implementation but with the same result.
