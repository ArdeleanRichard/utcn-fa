...
In this session you are required to implement and analyse BFS (Breadth-First Search), as presented in subsection 22.2 of \cite{cormen}.
...
In \texttt{bfs.cpp}, you have to complete the function \texttt{bfs()} which receive as arguments a pointer to a structure of type \texttt{Graph} and the source node \texttt{s} of type \texttt{Node*}. The function will implement BFS, as specified in the algorithm from the book (see subsection 22.2 from \cite{cormen}).
